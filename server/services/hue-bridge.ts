...
function isHueApiError(
  err: unknown,
): err is Error & { getHueErrorType(): number } {
  return (
    err instanceof Error && typeof (err as any).getHueErrorType === 'function'
  )
}
...
export class HueBridgeService {
  ...
  async discoverBridges(vlanIps: string[] = []): Promise<Bridge[]> {
    ...
    for (const [id, ip] of candidates.entries()) {
      const existing = await this.storage.getBridgeById(id)
      if (existing) {
        existing.ip = ip
  ...
  async pairBridge(bridge: Bridge): Promise<boolean> {
    logger.info('Attempting to pair with bridge', {
      bridgeId: bridge.id,
      ip: bridge.ip,
    })
    try {
  ...
  async pairWithLinkButton(
    ...
    try {
      ...
      await this.storage.insertBridge({
        id: ip,
        ip,
