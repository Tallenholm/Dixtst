...
export class MusicController {
  ...
  telemetry = async (
    req: Request<any, any, { roomId?: string; energy?: number; tempo?: number }>,
    res: Response
  ) => {
    ...
    await this.music.telemetry(String(roomId), Number(energy), typeof tempo === 'number' ? tempo : undefined)
    res.json({ ok: true })
  }
}
