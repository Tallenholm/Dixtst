...
export async function listOverrides(storage:IStorage){ const all=await storage.getAllSettings(); const now=Date.now(); const expired:string[]=[]; const active=all.filter(s=>String(s.key).startsWith('override:')).filter(s=>{const u=s.value?.until?Date.parse(s.value.until):undefined; if(u&&u<=now){expired.push(s.key);return false;} return !u||u>now}); await Promise.all(expired.map(k=>storage.deleteSetting(k))); return active }
